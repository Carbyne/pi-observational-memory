...
describe("normalizeSettingsConfig: worker watchdog / retry", () => {
	it("defaults: wall 20min, idle off, progress-idle 5min, retry off", () => {
		expect(DEFAULTS.workerTimeoutMs).toBe(1_200_000);
		expect(DEFAULTS.workerIdleTimeoutMs).toBe(0);
	...
	it("normalizes the progress-idle + retry knobs, accepting 0 where meaningful", () => {
